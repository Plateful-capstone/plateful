...
// Event listener for search button for searchDb
    searchButton.addEventListener("click", (e) => {
        ...
        fetch(url, {
            ...
            .then(async data => {
                await displaySearchResults(data); // Call the helper function to display search results
                executeSpoonacularSearch(searchInput, data); // Call the spoonacular search function after searchDb results are displayed
            })
            .catch(error => {
                console.error(error);
            });
...
// Function to execute Spoonacular search
    const executeSpoonacularSearch = (searchValue, ourResults) => {
        console.log("inside executeSpoonacularSearch, ourResults: ", ourResults);
        ...
        fetch(searchURL)
            ...
            .then((data) => {
                ...
                data.results.forEach((result, index) => {
                    // Check against EVERY item in ourResults to see if the spoonacularId matches.
                    // If it does match, don't add it to the results container.
                    let found = false;
                    ourResults.forEach((ourResult) => {
                        if (parseInt(ourResult.spoonacularId) === result.id) {
                            console.log("checking...")
                            console.log(`Our result: ${ourResult.recipeName}`)
                            console.log('Spoonacular result: ' + result.title)
                            found = true;
                        }
                    });
                    if (!found) {
